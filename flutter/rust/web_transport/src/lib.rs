...
async fn claim(
    ...
    let node_id = ticket_addr.id.to_string();
                        ...
                        while let Some(frame) = try_parse_frame(&mut pending) {
                            match frame.typ {
                                TYPE_DATA => event_tx
                                    ...
                                    .await
                                    .ok(),
                                TYPE_ATTACHED => match parse_attached(&frame.payload) {
                                    Some(echoed) if echoed == token => event_tx
                                        .try_send(SessionEvent::Attached {
                                            token_hex: hex(&echoed),
                                        })
                                        .ok(),
                                    _ => {
                                        event_tx
                                            .send(SessionEvent::Error {
                                                code: 0x02,
                                                message:
                                                    "host confirmed a different session identity"
                                                        .to_string(),
                                            })
                                            .await
                                            .ok();
